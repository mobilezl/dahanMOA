...
@interface ProcessAddViewController ()

@end

@implementation ProcessAddViewController
...
//    [self send_SearchRedirectTosByKey_Msg:@"钟"];
    ...
    if(UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad){
        textView = [[UITextView alloc] initWithFrame:CGRectMake(10, self.view.frame.size.height-(228+addHeight), self.view.frame.size.width-40, 50)];
    }else{
        textView = [[UITextView alloc] initWithFrame:CGRectMake(10, self.view.frame.size.height-(228+addHeight), 280, 50)];
    ...
    self.placeholderLabel.text = @"您的加签意见";
...
-(void) clickAgreeBtn:(id)sender
{
    ...
    if (deleteSpecificRows)
    {
        ...
        for (NSIndexPath *selectionIndex in selectedRows)
        {
            ...
            SearchTableViewCell *cell = [self.myTableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:selectionIndex.row inSection:selectionIndex.section]];
            
            
            [arry addObject:cell.UserID];
...
//            [indicesOfItemsToDelete addIndex:selectionIndex.row];
        ...
        if(arry.count > 0){
            ...
            if([txtContext isEqualToString:@""]){
                UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"提示" message:@"请输入加签意见后执行确认" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil,nil];
                [av show];
                return;
            ...
            NSString *actionName = @"加签";
            int actionValue = 1;
            
            AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
            manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/plain"];
            NSString *url = [NSString stringWithFormat:@"%@/WorkflowService.ashx?Action=RedirectProcess&loginName=%@&accesc_Token=%@&openId=%@&ProcessSN=%@&RedirectAction=%@&RedirectValue=%i&RedirectComment=%@&RedirectTos=%@",SERVERADDRESS,Process.ApproverLoginName,gAuthorizeCode,gOpenID,Process.ProcessSN,actionName,actionValue,textView.text,str];
            ...
            [manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                {
                    //            NSLog(@"------同意------成功");
                    [self.view.window showHUDWithText:@"加签成功" Type:ShowPhotoYes Enabled:YES];
            ...
            UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"提示" message:@"请选择加签人" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil,nil];
            [av show];
...
Approval * Process=nil;
-(void) passValue:(id)value {

    Process = value;
NSLog(@"-----approval = %@-----",Process.ProcessSN);
}
...
- (void) send_SearchRedirectTosByKey_Msg:(NSString *)key
{
    ...
    url = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSLog(@"send_SearchRedirectTosByKey_Msg_url:%@",url);
    [manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
