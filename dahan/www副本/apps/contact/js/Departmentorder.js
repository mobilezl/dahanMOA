...
function onDeviceReady() {
	if (Client == 1) {
		...
		DepartmentorderId = eval("("+getParameters()+")");
			var url = UrlRoot+"ContactsService.ashx?Action=GetDeptContactsByDeptId&userID="+ UserInfo.UserId+ "&authorizeCode="+ UserInfo.AuthorizeCode+"&departmentID="
...
function Search() {
	...
	if (SearchName == null || SearchName == "") {
		url = UrlRoot+"ContactsService.ashx?Action=GetDeptContactsByDeptId&userID="+ UserInfo.UserId+ "&authorizeCode="+ UserInfo.AuthorizeCode+"&departmentID="
				+ DepartmentorderId.DepartmentorderId;
	} else {
		if (!isNaN(SearchName)) {
			url = UrlRoot+"ContactsService.ashx?Action=SearchContactsByKey&userID="+ UserInfo.UserId+ "&authorizeCode="+ UserInfo.AuthorizeCode+"&key=Ext2*"
					+ SearchName;
		} else if ((/[\u4e00-\u9fa5]+/).test(SearchName)) {
			url = UrlRoot+"ContactsService.ashx?Action=SearchContactsByKey&userID="+ UserInfo.UserId+ "&authorizeCode="+ UserInfo.AuthorizeCode+"&key=userName*"
					+ SearchName;
		} else if ((/[a-z]|[A-Z]+/).test(SearchName)) {
			url = UrlRoot+"ContactsService.ashx?Action=SearchContactsByKey&userID="+ UserInfo.UserId+ "&authorizeCode="+ UserInfo.AuthorizeCode+"&key=userId*"
					+ SearchName;
		}
	}
	getJsonFormServer(url, onSuccess);
